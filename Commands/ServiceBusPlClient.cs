...
{
    public class ServiceBusPlClient:IServiceBusPlClient
    {
        ...
        public ServiceBusPlClient(IStringHelper datetimeParse, IHttpClientFactory factory, IConfiguration configuration, ILogger<ServiceBusPlClient> logger, IGraphClient graphClient)
        {
            ...
            _serviceBusProcessor = _serviceBusClient.CreateProcessor(_configuration[Constants.AppConfiguration.PLQueue], _serviceBusProcessorOptions);
        ...
        public async Task Handle(IApplicationBuilder serviceProvider)
        {
            try
            {
                ...
                _logger.LogWarning($"{nameof(ServiceBusPlClient)} Service Started");
