...
{
    public class ServiceBusInsuranceClient:IServiceBusInsuranceClient
    {
        ...
        public ServiceBusInsuranceClient(IStringHelper datetimeParse, IHttpClientFactory factory, IConfiguration configuration, ILogger<ServiceBusInsuranceClient> logger, IGraphClient graphClient)
        {
            ...
            _serviceBusProcessor = _serviceBusClient.CreateProcessor(_configuration[Constants.AppConfiguration.InsuranceQueue], _serviceBusProcessorOptions);
        ...
        public async Task Handle(IApplicationBuilder serviceProvider)
        {
            try
            {
                ...
                _logger.LogWarning($"{nameof(ServiceBusInsuranceClient)} Service Started");
        ...
        private async Task ProcessMessageAsync(ProcessMessageEventArgs args)
        {
            try
            {
                ...
                await args.CompleteMessageAsync(args.Message).ConfigureAwait(true);
            }
            catch (Exception ex)
            {
