...
{
    public class ServiceBusUpiClient:IServiceBusUpiClient
    {
        ...
        public ServiceBusUpiClient(IStringHelper datetimeParse, IHttpClientFactory factory, IConfiguration configuration, ILogger<ServiceBusUpiClient> logger, IGraphClient graphClient)
        {
            ...
            _serviceBusProcessor = _serviceBusClient.CreateProcessor(_configuration[Constants.AppConfiguration.UpiQueue], _serviceBusProcessorOptions);
        ...
        public async Task Handle(IApplicationBuilder serviceProvider)
        {
            try
            {
                ...
                _logger.LogWarning($"{nameof(ServiceBusUpiClient)} Service Started");
