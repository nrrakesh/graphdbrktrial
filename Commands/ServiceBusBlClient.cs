...
{
    public class ServiceBusBlClient:IServiceBusBlClient
    {
        ...
        public ServiceBusBlClient(IStringHelper datetimeParse, IHttpClientFactory factory, IConfiguration configuration, ILogger<ServiceBusBlClient> logger, IGraphClient graphClient)
        {
            ...
            _serviceBusProcessor = _serviceBusClient.CreateProcessor(_configuration[Constants.AppConfiguration.BLQueue], _serviceBusProcessorOptions);
        ...
        public async Task Handle(IApplicationBuilder serviceProvider)
        {
            try
            {
                ...
                _logger.LogWarning($"{nameof(ServiceBusBlClient)} Service Started");
        ...
        private async Task ProcessMessageAsync(ProcessMessageEventArgs args)
        {
            ...
            {
                _logger.LogError($"{ex.Message} {args.Message.Body}");
            }
        }
    }
}
