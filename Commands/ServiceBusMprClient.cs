...
{
    public class ServiceBusMprClient:IServiceBusMprClient
    {
        ...
        public ServiceBusMprClient(IStringHelper datetimeParse, IHttpClientFactory factory, IConfiguration configuration, ILogger<ServiceBusMprClient> logger, IGraphClient graphClient)
        {
            ...
            _serviceBusProcessor = _serviceBusClient.CreateProcessor(_configuration[Constants.AppConfiguration.MPRQueue], _serviceBusProcessorOptions);
        ...
        public async Task Handle(IApplicationBuilder serviceProvider)
        {
            try
            {
                ...
                _logger.LogWarning($"{nameof(ServiceBusMprClient)} Service Started");
